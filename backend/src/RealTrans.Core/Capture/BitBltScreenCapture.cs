...
namespace Translumo.Services
{
    public class BitBltScreenCapture : IScreenCapturer
    {
        ...
        public byte[] CaptureScreen(RectangleF captureArea) => MakeScreenshotInternal(captureArea, 1);

        public void Dispose() { }

        private byte[] MakeScreenshotInternal(RectangleF captureArea, int curAttempt)
        {
            IntPtr hdcSrc = IntPtr.Zero, hdcDest = IntPtr.Zero, hBitmap = IntPtr.Zero;
            try
            {
            ...
            {
                if (curAttempt >= CaptureAttempts) throw new CaptureException("Failed to capture screen", e);
                Thread.Sleep(AttemptDelayMs);
                return MakeScreenshotInternal(captureArea, ++curAttempt);
            }
            finally
            {
                if (hdcDest != IntPtr.Zero) Win32Interfaces.DeleteDC(hdcDest);
                if (hdcSrc != IntPtr.Zero) Win32Interfaces.ReleaseDC(IntPtr.Zero, hdcSrc);
                if (hBitmap != IntPtr.Zero) Win32Interfaces.DeleteObject(hBitmap);
            }
        }
    }
}
